package com.dmiranda.springcloud.app.gateway;
...
public class MsvcGatewayServerApplication {
	...
	RouterFunction<ServerResponse> routerConfig() {
		return route("msvc-products").route(
			path("/api/products/**"), http()
		).filter((request, next) -> {
			ServerRequest requestModified = ServerRequest.from(request)
				.header("message-request", "Mensaje...").build();
			ServerResponse response = next.handle(requestModified);
			response.headers().add("message-response", "Otro mensaje...");
			return response;
		}).filter(
			lb("msvc-products")
		).filter(
